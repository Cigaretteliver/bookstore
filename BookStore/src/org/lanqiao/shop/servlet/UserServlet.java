...
public class UserServlet extends HttpServlet {
	...
	public void regist(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 加密
		pwd = MyUtils.md5(pwd);
		//判断用户输入的是否是一个邮箱，如果是一个邮箱是邮箱的话使用邮箱注册，如果是用户名的话使用用户名注册。
		//使用正则判断是否是一个邮箱
		...
		if(flag){
			...
			//设置用户
			email.setAuthentication("cigaretteliver", "密码");
			try {
				//设置从哪个用户发出
	...
	//确认邮箱
	public void confirmEmail(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String capcha = request.getParameter("capcha");
		Object objStr = request.getSession().getAttribute("capcha");
		String cap = (String) objStr;
		//验证码正确
