			...
			$("#keyword").on("input propertychange",function(){
				...
				$.get(url,data,function(data){
					...
					$.each(eval(data),function(i,v){
						//加入列表
						$("<li class='hintKeyVal'>"+v.key+"</li>").on("click",function(){
							var val = $(this).text();
							$("#keyword").val(val);
						}).appendTo("#hint");
					});
  	...
  	<div class="container">
  		...
  		<div class="col-xs-9" class="container">
		  	<form action="book/listAll" method="get">
			  	<div>
			  		<input type="text" id="keyword" name="keyword" placeholder="请输入关键字" value="${requestScope.books.cretiera.keyword }">&nbsp;&nbsp;
			  		<!-- 搜索时的关键字提醒 -->
			  		<div id="hintDiv" style="width: 100px;">
			  			<ul id="hint">
			  			</ul>
			  		</div>
			  		价格：<input type="text" name="minprice" size="1" value="${requestScope.books.cretiera.minPrice==0? '':requestScope.books.cretiera.minPrice }">-
			  	...
			  	<div class="container">
				  	...
				  	<a href="book/listAll?keyword=${requestScope.books.cretiera.keyword }&minprice=${requestScope.books.cretiera.minPrice}&maxprice=${requestScope.books.cretiera.maxPrice}&orderby=${requestScope.books.cretiera.orderBy}&pageNo=${requestScope.books.cretiera.maxPageNo }">&nbsp;&nbsp;末页</a>&nbsp;&nbsp;跳至<input type="number" name="pageNo" size="1" value="${requestScope.books.cretiera.pageNo }">页<input type="submit" value="确定">
			  	</div>
		  	</form>
